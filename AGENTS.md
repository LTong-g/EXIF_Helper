...
- Android 安装包发布/分发归档中的 `<语义版本>` 以软件版本号为准；当前软件版本号为 `1.2.0`，对应 `app.json`、`package.json` 和 Android `versionName`。
...
- 元数据编辑必须作为独立导航页面实现；初版只编辑当前克隆流程支持的元数据字段，保存为新副本，不覆盖原照片。
- 元数据编辑处理 PNG 时必须沿用克隆流程的 PNG 输出选择：可继续保存 PNG，也可另存为 JPEG 副本。
